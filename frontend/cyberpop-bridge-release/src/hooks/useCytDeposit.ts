...
export const useCytDeposit = () => {
  ...
  return async (chainId: number, to: string, amount: number) => {
    const handler = await bridge?.callStatic._resourceIDToHandlerAddress(process.env.CytResourceID)
    ...
    const data = '0x' +
      ...
      to.substring(2);
    await bridge?.deposit(chainId, process.env.CytResourceID, data)
  }
}
