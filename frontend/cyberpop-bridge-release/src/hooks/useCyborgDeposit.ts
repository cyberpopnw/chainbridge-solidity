...
export const useCyborgDeposit = () => {
  ...
  return async (chainId: number, to: string, tokenId: unknown) => {
    const handler = await bridge?.callStatic._resourceIDToHandlerAddress(process.env.REACT_APP_CyborgResourceID)
    ...
    const data = '0x' +
      ...
      to.substring(2);
    return bridge?.deposit(chainId, process.env.REACT_APP_CyborgResourceID, data)
  }
}
